// Lightbox gallery + navbar smooth anchors
...
async function fetchVideos(){
  ...
  try{
    ...
    const exts = ['.mp4','.mov','.m4v','.webm'];
    files = all.filter(f => (f.mimeType && f.mimeType.startsWith('video/')) || exts.some(x => f.name?.toLowerCase().endsWith(x)));
    renderGallery();
...
function renderGallery(){
  gallery.innerHTML = '';
  if(!files.length){ statusEl.textContent = 'لا توجد فيديوهات حالياً.'; return; }
  for(const f of files){
    const card = document.createElement('div');
    card.className = 'card';
    const title = trimExt(f.name || 'بدون عنوان');
...
function openLightbox(index){
  current = index;
  loadCurrent(true); // محاولة ملء الشاشة بسرعة
...
function loadCurrent(tryFullscreen=false){
  player.innerHTML = '';
